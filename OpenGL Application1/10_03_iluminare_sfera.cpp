...
viewLocation,
projLocation,
lightColorLoc, lightPosLoc, viewPosLoc, matrUmbraLocation, codColLocation,
rendermode;
...
int const NR_PARR = 40, NR_MERID = 10;
...
//sursa de lumina
float xL =0.f, yL =100.f, zL = 100.f;
...
void CreateVBO(void)
{
	...
	for (int merid = 0; merid < NR_MERID; merid++)
	{
		for (int parr = 0; parr < NR_PARR + 1; parr++)
		{
			...
			Normals[index] = glm::vec3(x_vf, y_vf, v);
			//Colors[index] = glm::vec3(0.1f + sinf(u), 0.1f + cosf(v), 0.1f + -1.5 * sinf(u));
			//Colors[index] = glm::vec3(1.0f - u / radius, 0.1f + 0.9f * cosf(v), 0.2f + 0.1f * u / radius);
			Colors[index] = glm::vec3(0.5f, 0.1f , 0.1f );
			Indices[index] = index;
