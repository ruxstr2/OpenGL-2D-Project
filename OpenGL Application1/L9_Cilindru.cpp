...
float const U_MIN = 0, U_MAX = radius, V_MIN = -PI, V_MAX = PI;
...
void CreateVBO(void)
{
	...
	for (int merid = 0; merid < NR_MERID; merid++)
	{
		for (int parr = 0; parr < NR_PARR + 1; parr++)
		{
			...
			float x_vf =0.5* radius * cosf(v); // coordonatele varfului corespunzator lui (u,v)
			float y_vf =0.5* radius * sinf(v);
			float z_vf = radius-u;
...
void RenderFunction(void)
{
	...
	//cercul de la baza
	//for (int i = NR_PARR;i < (NR_PARR + 1) * NR_MERID;i += (NR_PARR+1) ) {
